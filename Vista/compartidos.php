...
<?php
  ...
  $listaarchivocargado = $objabmarchivocargado->buscar_compartido(null);
  ...
  ?>
  <h3>Mostrando Archivos Compartidos</h3>
  ...
  <table border="1">
      ...
      if (count($listaarchivocargado) > 0) {
        ...
        // foreach ($listaarchivocargadoestado as $objarchivocargadoestado) {

        //     echo '<tr><td value="' . $objarchivocargadoestado->getidestadotipos() .
        //      '" style="width:100px;">' . $objarchivocargadoestado->getidestadotipos() .
        ...
        foreach ($listaarchivocargado as $objarchivocargado) {
         ...
         // echo '<td> <input type="radio" onchange="capturar_radio()" id="datos" name="acnombre" value="' . $objarchivocargado->getacnombre() .'"><b>DEJAR DE COMPARTIR<b></td></tr>';
          echo '<td><a href="./eliminararchivocompartido.php?idarchivocargado='.$objarchivocargado->getidarchivocargado().'">Dejar de compartir</a></td>';
